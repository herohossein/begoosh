package com.act.voicecommand.Calendar;
...
//    String time = curDate.getHour() + ":"+curDate.getMinute()+":"+curDate.getSecond();
  ...
  public static String getDayMounthYear_voroodi(int Rooz,int Maah,int Saal){
    String rooz_str,Maah_str;
    if(Rooz<10){
      rooz_str="0"+String.valueOf(Rooz);
    }else{
      rooz_str=String.valueOf(Rooz);
    }
    if(Maah<10){
      Maah_str="0"+String.valueOf(Maah);
    }else{
      Maah_str=String.valueOf(Maah);
    }
    String this_date=String.valueOf(Saal)+"/"+Maah_str+"/"+rooz_str;
    return ShamsiCalendar.weekDayName(ShamsiCalendar.dayOfWeek(this_date))+ " "+
            ShamsiCalendar.monthDayName(Rooz)+
            ShamsiCalendar.monthName(Maah) +" ماه "  +
            String.valueOf(Saal);
  }
